 ...
 *
 * Authentication is enabled only when APP_PASSWORD env var is set.
 * When not set, all requests are allowed (local development).
 *
 * Session: HMAC-SHA256 signed token stored in httpOnly cookie.
 * Token validity: 30 days. Secret rotates on server restart.
 */
...
function validateToken(token: string, password: string): boolean {
  try {
    ...
    if (isNaN(tokenTime) || Date.now() - tokenTime > SESSION_MAX_AGE_MS) return false;
...
<head>
  ...
  <title>Thinktank - Login</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      min-height: 100vh; display: flex; align-items: center; justify-content: center;
      background: #0a0a0f; font-family: 'Segoe UI', 'Helvetica Neue', Arial, sans-serif; color: #e0e0e0;
    }
    .login-card {
      background: rgba(20, 20, 35, 0.9); border: 1px solid rgba(100, 100, 255, 0.15);
    ...
    h1 {
      text-align: center; font-size: 24px; font-weight: 600; margin-bottom: 8px;
    ...
    input[type="password"]:focus { border-color: rgba(100,140,255,0.5); }
    button {
      width: 100%; padding: 12px; background: linear-gradient(135deg, #4a6cf7, #6a4cf7);
    ...
    .error {
      ...
      color: #ff6b6b; padding: 10px 14px; border-radius: 8px; font-size: 13px;
...
export function authRoutes(): Router {
  ...
  router.post('/login', express.urlencoded({ extended: false }), (req: Request, res: Response) => {
    ...
    res.status(401).send(loginPageHtml('Incorrect password'));
  });
...
export function authMiddleware() {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (token && validateToken(token, password)) return next();
