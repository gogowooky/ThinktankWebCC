...
課題の分解を求められた場合、subtaskCandidatesに最大5件の候補を提示する。idは英数字・ハイフン・アンダースコアの一意な値、titleは短い課題名、goalは目的、completionCriteriaは完了条件、reasonは分解理由。既存のsubtasksにある課題を重複提案しない。不明な完了条件は空文字でよい。提案は未採用であり自動作成しない。不要な場合とchat-onlyでは空配列。
