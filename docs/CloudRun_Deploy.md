...
手順1〜3・5〜6は **Google Cloud 側に保存される設定**であり、どの PC から
実行しても同じ場所に残る。一度どこかの PC で通せば、他の PC でやり直す必要はない。
PC 毎に必要なのは手順0だけ。

| 手順 | 単位 | 頻度 |
| --- | --- | --- |
| 0. gcloud のセットアップ | **PC毎** | その PC で初回のみ |
| 1. Secret Manager にシークレット登録 | プロジェクト | 全体で1回 |
| 2. ランタイムSAに secretAccessor 付与 | プロジェクト | 全体で1回 |
| 3. IAP 有効化・サービスエージェント作成 | プロジェクト | 全体で1回 |
| 4. デプロイ | どの PC からでも | 変更の度 |
| 5. IAP のIAMバインディング | プロジェクト | 全体で1回（利用者追加時のみ再実行） |
| 6. ログ確認・JWT検証の必須化 | プロジェクト | 全体で1回 |

Docker は不要。`--source .` は Cloud Build 上でビルドするため、ローカルに
Docker が無くてもデプロイできる。

なお、ローカル開発用の `server/.env` と鍵JSON は PC 毎に必要だが、これは
デプロイとは別系統でありこの手順書の対象外。

## 0. gcloud のセットアップ（PC毎に初回のみ）

その PC からデプロイするなら認証が必要。

```bash
gcloud auth login
```

さらに CLI の更新が要る。`--iap` フラグと `--resource-type=cloud-run` は
新しい CLI にしか無く、SDK 550.0.0 / core 2025.12.12 では未対応であることを
確認済み。**対話モードのターミナルで**実行すること（非対話では更新できない）。
