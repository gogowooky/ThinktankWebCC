...
graph TD
    ...
    AppLayout --> ASB[ApplicationStatusBarArea: 最下段ステータスバー]

    TTP --> TTR[ThinktankTabBar: 左縦型タブバー]
    TTP --> TTA[ThinktankArea: コンテンツ領域]
    TTA --> TTMR[ThinktankMenuRibbon: 上部メニューリボン/横型]
    TTA --> TTF[ThinktankFilterPanel: フィルターバー]
    TTA --> TTB[ThinktankFilterView: リスト表示領域]

    OP --> OPR[OverviewTabBar: 左縦型タブバー]
    OP --> OPA[OverviewArea: コンテンツ領域]
    OPA --> OPMR[OverviewMenuRibbon: 上部メニューリボン/横型]
    OPA --> OPF[OverviewFilterPanel: フィルターバー]
    OPA --> OPS[OverviewSearchBar: 検索バー]
    OPA --> OPB[OverviewAreaBody: コンテンツ表示領域]

    WP --> WRB[WorkoutTabBar: 左縦型タブバー]
    WP --> WSP[WorkoutSettingArea: 設定トレイ]
    WP --> WPT[WorkoutPanelTree: BSPツリー領域]
    WPT --> BSP[WorkoutArea: 個別ペイン]
    BSP --> WAR[WorkoutMenuRibbon: ペインヘッダー/横型]
    BSP --> WAB[WorkoutAreaBody: ペインコンテンツ領域]

    RTP --> RTA[ReThinkArea: コンテンツ領域]
    RTA --> RTMR[ReThinkMenuRibbon: 上部メニューリボン/横型]
    RTA --> RTCB[ReThinkContextBar: 連携中アイテム表示バー]
    RTA --> RTCH[ReThinkChat: チャット領域]
    RTP --> RTR[ReThinkTabBar: 右縦型タブバー]
```

#### ① 全体レイアウト (`ApplicationContainer` / `ApplicationLayout`)
*   **メインコンテナ (`ApplicationContainer`)**:
    *   画面全体を縦方向に制御し、パネル群を表示する `ApplicationLayout` と最下段のステータスバー `ApplicationStatusBarArea` を内包します。
*   **アプリ全体の表示領域 (`ApplicationLayout`)**:
    *   ステータスバーを除いた画面上部で、4つの主要パネルを横並びに制御します。
*   **仕切り領域 (`Splitter`)**:
    *   パネル間のリサイズを行うドラッグ領域。
*   **最下段ステータスバー (`ApplicationStatusBarArea`)**:
...
*   **コンテンツ領域 (`ThinktankArea`)**:
    ...
    *   **フィルター・検索領域 (`ThinktankFilterPanel` / `ThinktankSearchBar`)**: キーワード、更新・作成日時、ContentType（メモ/Thought/テーブル等）によるフィルタ入力欄。
    *   **データリスト領域 (`ThinktankFilterView`)**: フィルタ結果のThink一覧がテーブル/カード形式で縦スクロール配置されます。
...
*   **コンテンツ領域 (`OverviewArea`)**:
    ...
    *   **フィルター・検索領域 (`OverviewFilterPanel` / `OverviewSearchBar`)**: 選択中Thought内のThinkに対するキーワード・日付フィルタ入力欄。
...
*   **メインコンテンツエリア**:
    *   **個別ペイン領域 (`WorkoutArea`)**: BSPツリーにより再帰分割された個別の編集・表示単位。
        *   **ペインヘッダー領域 (`WorkoutMenuRibbon`)**: ペインの上部（横型）。ドラッグハンドル、タイトル、保存状態（dirty）インジケータ、MediaType切替タブが配置されます。
...
*   **コンテンツ領域 (`ReThinkArea`)**:
    ...
    *   **コンテキスト表示バー (`ReThinkContextBar`)**: 現在連携・同期している Thought または Think の名前を表示するヘッダーバー。
    *   **チャット表示領域 (`ReThinkChat`)**: AIとの CLI ターミナル風チャット対話ログおよびコマンド入力欄。

##### 5. 最下段ステータスバー (ApplicationStatusBarArea - アプリ全体最下部)
*   **配置箇所**: アプリケーション全体の最下部に常時固定表示（高さ 28px）。
*   **左端**:
    *   **起動モードインジケータ (`StatusBarModeIndicator`)**: 現在の起動モード（Local / Online）を示すアイコンを表示。
    *   **テキスト入力エリア (`StatusBarTextInput`)**: 現在選択されているモードに応じた入力・表示処理を行います。
*   **右端**:
    *   **モード切替ボタン群 (`StatusBarModeButton*`)**: ステータス表示、単語ハイライト、キーアクション履歴、CLIコマンドラインなどのビューモード切り替えボタンが配置されます。
...
**iPhone 表示時の既定** (260827)：`navigator.userAgent` が iPhone / iPod の場合、起動時に簡易モード（`compact`）と Editor の WordWrap を強制します。保存済み UI 状態より優先されます（`src/utils/deviceInfo.ts` / `src/main.tsx` / `src/App.tsx`）。

### 1.3 起動モードおよび同期設定のUI表示仕様
本システムは、動作環境に応じて異なる起動モード（Local / Online）で動作し、それに応じたインジケータおよび設定コントロールをUI上に配置します。

#### 起動モードインジケータの表示
*   **配置箇所**: 最下段ステータスバー（`ApplicationStatusBarArea`）の左端。
*   **アイコン表示仕様**:
...
各ペインのヘッダーバー（`<WorkoutMenuRibbon>`）のドラッグハンドルをマウスでホールドすると、ペインのドラッグ移動状態（Swapドラッグ）が開始されます。
