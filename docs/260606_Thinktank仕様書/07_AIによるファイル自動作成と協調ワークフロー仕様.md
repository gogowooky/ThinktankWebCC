...
AIは対話の文脈に応じて、以下の単機能（アトミック）なツールを組み合わせて呼び出す。

1.  **`saveThink(id, category, title, content)`**
    *   **役割**: 個別のデータアイテム（`memo` または `links` 等のThinkファイル）をDBに直接作成・更新する。
    *   **引数**:
        *   `id`: `yyyy-MM-dd-HHmmss-memo` または `yyyy-MM-dd-HHmmss-links` 形式（AI自身が時間加算して生成する）。
        *   `category`: `"memo"` または `"links"`。
        *   `title`: 画面一覧に表示されるファイルのタイトル。
        *   `content`: Markdown形式のファイル本文。
2.  **`saveThought(id, title, content)`**
    *   **役割**: 親フォルダに相当するThought（`category: 'thought'`）を作成・更新する。
    *   **引数**:
        *   `id`: `yyyy-MM-dd-HHmmss-thought` 形式。
        *   `title`: Thoughtのタイトル（主題）。
        *   `content`: 紐付けるThinkファイルのIDリストを含む本文（フォーマット: `[Title]\n* [think-id-1]\n* [think-id-2]`）。

### 2.2 IDの自動決定と並列処理プロセス（解決策C）

1.  **AIによるID設計**:
    *   AIはサーバーから与えられるプロセス開始時刻を基準に、**1秒ずつ進めた重複しない日時ID** を自律的に割り振る。これにより、1ターンのレスポンス内で親子ファイルを同時に定義できる。
2.  **Parallel Tool Callの集約実行**:
    *   バックエンド（`ChatService.ts`）は、AIから同一ターンに返された複数のツール呼び出し（`saveThink` や `saveThought`）を配列として集約し、BigQueryに並行して保存（`bigqueryService.save(record)`）する。
3.  **リアルタイムクライアント連携**:
