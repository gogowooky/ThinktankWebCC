...
let isFoldingActionsRegistered = false;

function registerFoldingActions() {
  if (isFoldingActionsRegistered) return;
  isFoldingActionsRegistered = true;

  TTActions.Register({
    ActionID: 'TextEditor.Folding.Forward',
    Completion: (item) => {
      const ed = TTShortcutManager.instance.activeEditor;
      if (!ed) return;
      ...
      for (let i = currentLine - 1; i >= 1; i--) {
        if (model.getLineContent(i).match(/^(\s{0,3})(#{1,5})\s/)) {
          ed.setPosition({ lineNumber: i, column: 1 });
          ed.revealLineInCenter(i);
          item.Result = `Moved to line ${i}`;
          item.Allow = true;
          return;
        }
      }
  ...
  TTActions.Register({
    ...
    Completion: (item) => {
      ...
      const L = match[2].length;
      const hiddenAreas = ed.getHiddenAreas() || [];
      const isCollapsed = (line: number) => {
        const nextLine = line + 1;
        return hiddenAreas.some((r: any) => r.startLineNumber <= nextLine && nextLine <= r.endLineNumber);
      };

      if (isCollapsed(currentLine)) {
        ed.trigger('keyboard', 'editor.unfold', null);
        item.Result = 'Unfolded self';
        item.Allow = true;
      } else {
        const lineCount = model.getLineCount();
        let endLine = lineCount;
        for (let i = currentLine + 1; i <= lineCount; i++) {
          const m = model.getLineContent(i).match(/^(\s{0,3})(#{1,5})\s/);
          if (m && m[2].length <= L) {
            endLine = i - 1;
            break;
          }
        ...
        const subHeaders: { line: number; level: number }[] = [];
        for (let i = currentLine + 1; i <= endLine; i++) {
          const m = model.getLineContent(i).match(/^(\s{0,3})(#{1,5})\s/);
          if (m) {
            subHeaders.push({ line: i, level: m[2].length });
          }
        }

        const closedSubHeaders = subHeaders.filter(h => isCollapsed(h.line));
        if (closedSubHeaders.length > 0) {
          const minLevel = Math.min(...closedSubHeaders.map(h => h.level));
          const targets = closedSubHeaders.filter(h => h.level === minLevel);

          const monaco = (window as any).monaco;
          const originalSelection = ed.getSelection();
          targets.forEach(t => {
            ed.setSelection(new monaco.Selection(t.line, 1, t.line, 1));
            ed.trigger('keyboard', 'editor.unfold', null);
          });
          if (originalSelection) ed.setSelection(originalSelection);
          item.Result = `Unfolded ${targets.length} sub-headers at level ${minLevel}`;
          item.Allow = true;
        }
      }
    }
  ...
  TTActions.Register({
    ...
    Completion: (item) => {
      ...
      if (!isCollapsed(currentLine)) {
        // 自分自身が開いている場合：
        // 支配下の子孫で「開いているもの」を探して、あればそれを閉じる。なければ自分を閉じる。
        const openSubHeaders = subHeaders.filter(h => !isCollapsed(h.line));
        if (openSubHeaders.length > 0) {
          const maxLevel = Math.max(...openSubHeaders.map(h => h.level));
          const targets = openSubHeaders.filter(h => h.level === maxLevel);
          ...
          if (originalSelection) ed.setSelection(originalSelection);
          item.Result = `Folded ${targets.length} sub-headers at level ${maxLevel}`;
          item.Allow = true;
        } else {
          // 子孫がすべて閉じているなら、自分を閉じる
          ed.trigger('keyboard', 'editor.fold', null);
          item.Result = 'Folded self';
          item.Allow = true;
        }
      } else {
        // 自分自身がすでに閉じている場合、兄弟FoldingをすべてCloseにする
        // 1. 親見出し（レベルが L 未満の最初の見出し）を上方向に探索
        let P = 0;
        for (let i = currentLine - 1; i >= 1; i--) {
          const m = model.getLineContent(i).match(/^(\s{0,3})(#{1,5})\s/);
          if (m && m[2].length < L) {
        ...
        // 2. 親の配下（Pの支配範囲）の終端 E を特定
        let E = lineCount;
        if (P > 0) {
          const pMatch = model.getLineContent(P).match(/^(\s{0,3})(#{1,5})\s/);
          const pLevel = pMatch ? pMatch[2].length : 0;
          for (let i = P + 1; i <= lineCount; i++) {
            const m = model.getLineContent(i).match(/^(\s{0,3})(#{1,5})\s/);
            if (m && m[2].length <= pLevel) {
              E = i - 1;
              break;
            }
          }
        ...
        // 3. 支配範囲内（P + 1 〜 E、P=0なら 1 〜 lineCount）で、レベルがちょうど L の兄弟をリストアップ
        const startSearch = P > 0 ? P + 1 : 1;
        const siblings: number[] = [];
        for (let i = startSearch; i <= E; i++) {
          const m = model.getLineContent(i).match(/^(\s{0,3})(#{1,5})\s/);
          if (m && m[2].length === L) {
            siblings.push(i);
          }
        ...
        // 4. まだ Close されていない兄弟をすべて折り畳む
        const openSiblings = siblings.filter(line => !isCollapsed(line));
        if (openSiblings.length > 0) {
          const monaco = (window as any).monaco;
          const originalSelection = ed.getSelection();
          openSiblings.forEach(line => {
            ed.setSelection(new monaco.Selection(line, 1, line, 1));
            ed.trigger('keyboard', 'editor.fold', null);
          });
          if (originalSelection) ed.setSelection(originalSelection);
          item.Result = `Folded ${openSiblings.length} siblings`;
          item.Allow = true;
        } else {
          item.Result = 'Self and all siblings already folded';
          item.Allow = true;
        }
      }
    }
