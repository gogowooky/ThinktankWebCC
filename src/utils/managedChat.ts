...
export function parseManagedChatTitle(name: string) {
  const match = /^(TODO|PROJ|ASK|EVNT|LOOP):(Thinktank|Overview|Workout|ReThink)[｜|](.*)$/s.exec(name.trim());
  if (!match) return null;
  const state = /^\[([^\]]+)\]/.exec(match[3]);
  const known = state && (MANAGED_STATES as readonly string[]).includes(state[1]);
  return { kind: match[1], panel: match[2], state: known ? state[1] : '状態未設定', title: known ? match[3].slice(state![0].length).trim() : match[3].trim() };
}
