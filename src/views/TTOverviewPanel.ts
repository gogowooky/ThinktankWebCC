...
export class TTOverviewPanel extends TTUIItem {
   ...
   *  Status `OverviewPanel.Area.OpenWidth` の実体。px は panelAreaWidth.ts が導出する。 */
  public AreaWidthMode: AreaWidthMode = 'init';

  /** ユーザーが Splitter で設定した幅(px)。AreaWidthMode='user' のときに使う。 */
  public AreaUserWidth: number = INIT_AREA_WIDTH.Overview;
  ...
  public OpenBundle(bundleId: string, mediaType: MediaType = 'datagrid'): void {
    ...
    this.NotifyUpdated();
    TTUIStateManager.instance.notifyPropertyChanged('OverviewPanel.Bundle.ID');
  }
