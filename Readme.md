...
- 本アプリケーションをリファクタリングする目的で、AIに対する指令のみで本アプリケーションを再構成することを考えています。そのための指令書を作成してほしいのですが、AIにアプリを作成させる際に、多段階に分けて少しずつ０ベースで開発し、段階ごとに動作を確かめられる中間製品を作成しながらすすめたいと考えています。段階的な開発とすることで、途中の指令に機能拡張を追記する機会も生じ、アプリケーションを発展させる方法としても適切であると考えられます。そのような、何度もAIに依頼しながら段階的に本アプリケーションを作成するため、第一指令から段階ごとにAIと対話してアプリを完成させるための指令書群を作成してください。　段階が関連する機能単位で区切られていると、わかりやすくなりますのでそのような方法でお願いいたします。

以下についても実装してください。
・　スマートホンがクライアントであるときの表示とスマートホンUIイベントへの対応
・　AIとの対話をMemoとは別で管理する
・　データベースはTTCollection毎に分ける
・　現在のBigQueryのテーブルは使用せずに開発指令を記載してください。
・　ドラッグアンドドロップされたファイルはGoogleDriveのyyyy-mm-ddフォルダに格納。
・　規定のタイトルのGMailアイテムについて読み取り、本アプリで管理する
・　段階の数は３００段以下程度でお願いします
